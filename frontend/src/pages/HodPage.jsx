...
const GatepassDetailsView = ({ gatepassId, onBack }) => {
  ...
  const handleRejectConfirm = async () => {
    ...
    setDeciding(true);
    try {
      const res = await decideHodGatepass({ gatepassId, decision: 'rejected', rejectionReason });
  ...
  return (
    <div className="os-section">
      ...
      <div className="os-history-table">
        {gatepasses.map((gp) => (
          <div key={gp._id} className={`os-history-row ${gp.stageStatus?.hod?.status}`}>
            ...
            <div className={`os-status-badge ${gp.stageStatus?.hod?.status}`}>
